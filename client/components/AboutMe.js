...
export default function AboutMe() {
    return (
        <motion.div id="about" className='min-h-full w-full bg-black flex relative flex-col items-center justify-center'>
            <motion.img src="/rocket.svg" alt="" initial={{ x: -550 }} transition={{ duration: 1, type: "spring" }} whileInView={{ x: 0 }} className="w-[500px] hidden lg:inline absolute top-[40px] left-[50px] animateRocket" />
            ...
            <motion.div className='absolute bottom-[30px]  lg:top-[30px] right-[0px]' onViewportEnter={(e) => {
                ...
                <motion.img src="/running.svg" alt="" transition={{ duration: 1 }} className="w-[60vw] max-w-[500px] lg:-rotate-90" />
            </motion.div>
        </motion.div>
    )
}
